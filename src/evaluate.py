...
class EvaluateStrategies:
    ...
    def evaluate_drl_agents(self):
        ...
        drl_df.to_csv(
            os.path.join(self.results_dir, 'drl_evaluation_results.csv'),
            index=False
        )
    ...
    def statistical_significance_test(self, drl_df: pd.DataFrame, benchmark_df: pd.DataFrame):
        ...
        # Group by agent
        groups = [group['annual_return'].values for name, group in all_results.groupby('agent')]
        group_names = list(all_results.groupby('agent').groups.keys())
        ...
        tukey_df.to_csv(
            os.path.join(self.results_dir, 'tukey_hsd_results.csv'),
            index=False
        )
    ...
    def create_comparison_table(self, drl_df: pd.DataFrame, benchmark_df: pd.DataFrame):
        ...
        for agent in summary.index:
            ...
            for metric in ['annual_return', 'sharpe_ratio', 'sortino_ratio', 'max_drawdown', 'cvar_5']:
        ...
        comparison_df.to_csv(
            os.path.join(self.results_dir, 'comparison_table.csv'),
            index=False
        )
