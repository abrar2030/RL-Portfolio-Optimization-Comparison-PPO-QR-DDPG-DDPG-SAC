...
class BacktestBenchmark:
    ...
    def backtest_strategy(self, strategy_name: str, lookback_window: int = 60) -> Dict:
        """
        ...
        for i in range(1, len(self.dates)):
            ...
            if i % self.rebalance_freq == 0 and i >= lookback_window:
                ...
                returns_df = pd.DataFrame(
                    np.array(returns_data).T, columns=self.tickers
                )
                ...
                transaction_cost = (
                    np.sum(weight_changes) * portfolio_value * self.transaction_cost_pct
                )
